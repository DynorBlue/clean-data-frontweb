...
export const registroCiudadano = (data) => api.post('/ciudadanos/registro', data);
...
window.resetCiudadanoModal = async () => {
    document.getElementById('ciudadanoModalTitle').textContent = 'Nuevo Ciudadano';
    ...
    const colonias = await api.get('/colonias');
    document.getElementById('ciudadanoIdColonia').innerHTML = '<option value="">Seleccionar colonia...</option>' +
...
window.editarCiudadano = async (id) => {
    try {
        ...
        document.getElementById('ciudadanoModalTitle').textContent = 'Editar Ciudadano';
        document.getElementById('ciudadanoId').value = ciudadano.idPersona;
        document.getElementById('ciudadanoNombre').value = ciudadano.persona?.nombre || '';
        document.getElementById('ciudadanoTelefono').value = ciudadano.persona?.telefono || '';
        document.getElementById('ciudadanoEmail').value = '';
        document.getElementById('ciudadanoPassword').value = '';
        document.getElementById('ciudadanoDireccion').value = ciudadano.direccionCalle || '';
        ...
        coloniaSelect.innerHTML = `<option value="">Seleccionar colonia...</option>` +
            coloniasList.map(c => `<option value="${c.idColonia}" ${c.idColonia === ciudadano.colonia?.idColonia ? 'selected' : ''}>${c.nombre}</option>`).join('');
        
        window.ciudadanoModalMode = 'edit';
        
        const modal = new bootstrap.Modal(document.getElementById('ciudadanoModal'));
        modal.show();
    } catch (error) {
...
window.guardarCiudadano = async () => {
    try {
        ...
        const direccionCalle = document.getElementById('ciudadanoDireccion').value.trim();
        ...
        if (id && window.ciudadanoModalMode === 'edit') {
            const data = { direccionCalle: direccionCalle || null, persona: { nombre, telefono: telefono || null }, idColonia: parseInt(idColonia) };
            await updateCiudadano(id, data);
            showToast('Ciudadano actualizado correctamente', 'success');
        } else {
            if (!email || !password) {
                showToast('Email y contraseña son requeridos para nuevo ciudadano', 'warning');
                return;
            }
            const data = { nombre, telefono: telefono || null, email, password, direccionCalle: direccionCalle || null, idColonia: parseInt(idColonia) };
            await registroCiudadano(data);
            showToast('Ciudadano registrado correctamente', 'success');
        }
        
        const modal = bootstrap.Modal.getInstance(document.getElementById('ciudadanoModal'));
        modal.hide();
